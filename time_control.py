...
def Available():
    ...
    if co2port in ports:
        ...
        #time.sleep(4)
        end_T = time.time()#time.time_ns()
        print("Time taken", end_T-star_T, "ns")
        return True
...
def Temporizador(minutos):
    ...
    for i in arrTempo:
        ...
        #print(i)
    endt=time.time()
    print('Total time:', (endt-start)/60)
    return True
...
###Leemos el sensor de CO2
    ...
    if Temporizador(t_user):
        
        print('Iniciar medicion de sensores')
    ...
    if Available ():
        ...
        print('Verify sensors conection')
